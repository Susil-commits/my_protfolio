...
export default function Chatbot() {
  ...
  // Pure in-memory session state: Always start clean on every visit / reload
  ...
  // Initialize Speech Recognition & Synthesis and purge any stale legacy storage
  useEffect(() => {
    try {
      sessionStorage.removeItem('susil_portfolio_chat');
      localStorage.removeItem('susil_portfolio_chat');
    } catch {
      // ignore
    }
            ...
            <div className="flex items-center gap-1 text-[var(--theme-mist)]">
              {messages.length > 0 && !isMinimized && (
                <button
                  ...
                  className="px-2 py-1 rounded-lg bg-white/[0.04] hover:bg-cyan-500/15 border border-white/10 hover:border-cyan-500/30 hover:text-cyan-300 transition-all cursor-pointer flex items-center gap-1 text-xs"
                >
