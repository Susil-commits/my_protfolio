      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="reveal-on-scroll text-mist mt-4 max-w-lg mx-auto text-sm">
            Full-stack platforms I've designed, built, and shipped end-to-end with deep system optimizations
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {projects.map((project, i) => (
            <SpotlightCard
              key={project.title}
              className="reveal-scale card-morph-border p-8 group relative overflow-hidden flex flex-col justify-between"
            ...
            >
              ...
              <div className="relative z-10 pt-4 border-t border-pearl/[0.04] space-y-3">
                {/* Architecture Deep Dive Button */}
                <button
                  type="button"
                  onClick={() => setSelectedArchProject(project.title)}
                  className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-semibold text-pearl/90 bg-pearl/[0.05] hover:bg-pearl hover:text-black border border-pearl/15 hover:border-pearl shadow-sm transition-all duration-300"
                >
                  {BlueprintIcon}
